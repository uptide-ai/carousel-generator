...
- **Click-outside deselects**: The outer `SlidesEditor` wrapper has `onClick={(e) => setCurrentSelection("", e)}`, so clicking the empty area around slides clears the current selection and the sidebar falls back to its last tab (Settings by default). Individual slide/element click handlers still call `setCurrentSelection(...)` with their own targets first; `stopPropagation` on interactive children prevents the outer handler from overwriting a real selection.
...
- **Title/Subtitle descender clipping**: Title and Subtitle elements apply `clipPath: "inset(0 0 0.2em 0)"` and compensate with `marginBottom: calc(... - 0.2em)` to trim ugly descender/line-gap space at the bottom of display fonts. `PageBase` uses `overflowClipMarginTop: "40px"` (top edge only) so the element menubar positioned at `-top-9` on the first element can escape the slide's clip upward, while content / images are still strictly clipped on the other sides and cannot bleed into neighboring slides. Trade-off: the change-type dropdown on the very last element can be clipped at the slide's bottom; we accept that because image bleed is the more visible problem.
